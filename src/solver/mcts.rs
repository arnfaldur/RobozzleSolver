...
use std::intrinsics::{sqrtf64, logf64};
...
impl Node {
    fn new(source: Source) -> Self { Node { source, scores: 0.0, rollouts: 0, children: vec![] } }
    fn is_leaf(&self) -> bool { self.children.is_empty() }
    fn uct(&self, parent_visits: u64) -> f64 {
        return if self.rollouts == 0 { INFINITY } else {
            self.scores / self.rollouts as f64 + EXPLORATION *
