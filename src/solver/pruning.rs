...
fn deny_inner(puzzle: &Puzzle, program: &Source, show: bool) -> bool {
    ...
    let mut invoked = [0; 5];
    let mut has_nops = false;
    let mut has_probe = [false; 5];
    ...
    for m in 0..5 {
        ...
        for i in 0..puzzle.methods[m] {
            ...
            if ins.is_function() {
                ...
                } else if only_cond[ins.source_index()]
                    != (ins.get_cond() | ins.with_loosened(true))
                {
            ...
            // has_forward |= ins.get_ins() == FORWARD || ins.is_nop() || ins.is_probe();
    ...
    for m in 0..5 {
        ...
        for i in 1..puzzle.methods[m] {
            ...
            if banned_pair(puzzle, a, b, show) {
                if show {
                    println!("de1");
                }
                return true;
...
fn banned_pair_inner(b: Ins, a: Ins, show: bool, puzzle: &Puzzle) -> bool {
    ...
    if a.is_turn() && b.is_turn() {
        banned |= a.as_vanilla() > b.as_vanilla(); // only let a series of turns have one color order
        if banned {
            if show {
    ...
    if a.is_mark() && b.is_mark() {
        ...
        banned |= a.get_mark_as_cond() == b.get_cond()
            && a.get_cond() != b.get_mark_as_cond()
            && a.get_cond() != b.get_cond();
        if banned {
            if show {
    ...
    if a.is_gray() && a.is_mark() && !b.is_cond(a.get_mark_as_cond()) {
        if show {
...
pub fn banned_trio(puzzle: &Puzzle, a: Ins, b: Ins, c: Ins, show: bool) -> bool {
    ...
    banned |=
        a.is_mark() && a.is_gray() && b.is_order_invariant() && !c.is_cond(a.get_mark_as_cond());
